...
TEST(Compress, EmptyInputGivesEmpty) {
  ...
  libcompressor_Buffer out2 =
      libcompressor_compress(libcompressor_Bzip, null_input);
  ...
  libcompressor_Buffer out4 =
      libcompressor_compress(libcompressor_Bzip, zero_input);
  EXPECT_EQ(out4.data, nullptr);
  EXPECT_EQ(out4.size, 0);

  libcompressor_Buffer empty_str{const_cast<char *>(""), 0};
  libcompressor_Buffer out5 =
      libcompressor_compress(libcompressor_Zlib, empty_str);
...
TEST(Compress, BzipTestStringExactBytes) {
  ...
  const std::vector<unsigned char> expected = {
      ...
      0x4a, 0x7c, 0x69, 0x05, 0x00, 0x00, 0x04, 0x83, 0x80, 0x00,
      0x00, 0x82, 0xa1, 0x1c, 0x00, 0x20, 0x00, 0x22, 0x03, 0x68,
      0x84, 0x30, 0x22, 0x50, 0xdf, 0x04, 0x99, 0xe2, 0xee, 0x48,
      0xa7, 0x0a, 0x12, 0x09, 0x4f, 0x8d, 0x20, 0xa0};
